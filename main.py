...
def main_pca(dataset_train, scaler_model, window_type, window_size, window_stride, pca_components,
             show_plot_variance=True, threshold_method='quantile', dataset_eval=None, dataset_test=None):
    """
    ...
    train_errors = (np.square(dataset_train_process.ts_data.data -
                              dataset_train.ts_data.data).mean(axis=1))
    if threshold_method == 'quantile':
        pc_lower_bound, pc_upper_bound = compute_quantile_error_threshold(
            errors=train_errors, lower_perc=0.05, upper_perc=0.95)
    elif threshold_method == 'svm':
        one_class_model = OneClassSVM(gamma="auto").fit(
        ...
        local_outlier_factor_model = LocalOutlierFactor(
            n_neighbors=100, novelty=True, contamination='auto').fit(train_errors.reshape(-1, 1))

    plot_ts(title="PCA Figure Train", ts_name="train",
            ts={'start': dataset_train.ts_data.data, 'end': dataset_train_process.ts_data.data},
            features=TimeSeriesUtils.PLOT_FEATURES,
            n_rows=2, n_cols=3, figsize=(15, 5), colors={'start': 'black', 'end': 'orange'}, save_plot=True,
            subfolder=f"pca_{threshold_method}")
    ...
    if dataset_eval is not None:
        ...
        for ts_name, ts in dataset_eval_process.time_series.items():
            plot_ts(title=f"PCA Figure Eval {ts_name}", ts_name=f"eval_{ts_name}",
                    ts={'start': dataset_eval.time_series[ts_name].data,
                        'end': dataset_eval_process.time_series[ts_name].data}, features=TimeSeriesUtils.PLOT_FEATURES,
                    n_rows=2, n_cols=3, figsize=(15, 5), colors={'start': 'black', 'end': 'orange'}, save_plot=True,
                    subfolder=f"pca_{threshold_method}")

            eval_errors = (np.square(
                dataset_eval_process.time_series[ts_name].data - dataset_eval.time_series[ts_name].data).mean(axis=1))

            eval_anomalies_mask = None
            if threshold_method == 'quantile':
                eval_anomalies_mask = ((eval_errors > pc_upper_bound) | (
                        eval_errors < pc_lower_bound)) == True
            elif threshold_method == 'svm':
                eval_anomalies_mask = one_class_model.predict(
                    eval_errors.reshape(-1, 1)) == -1
            elif threshold_method == 'local_outlier_factor':
                eval_anomalies_mask = local_outlier_factor_model.predict(
                    eval_errors.reshape(-1, 1)) == -1
            ...
            if eval_anomalies_mask is not None:
                ...
                anomalies = copy.deepcopy(
                    dataset_eval.time_series[ts_name].data)
                anomalies[~eval_anomalies_mask] = np.nan

                plot_ts(title=f"PCA Figure Eval Anomalies {ts_name}", ts_name=f"eval_anomalies_{ts_name}",
                        ts={'time_series': dataset_eval.time_series[ts_name].data, 'anomalies': anomalies},
                        features=TimeSeriesUtils.PLOT_FEATURES,
                        n_rows=2, n_cols=3, figsize=(15, 5), colors={'time_series': 'black', 'anomalies': 'red'},
                        markers={'anomalies': 'o'}, save_plot=True, subfolder=f"pca_{threshold_method}")
    # TEST SECTION
    if dataset_test is not None:
        ...
        for ts_name, ts in dataset_test_process.time_series.items():
            plot_ts(title=f"PCA Figure Test {ts_name}", ts_name=f"test_{ts_name}",
                    ts={'start': dataset_test.time_series[ts_name].data,
                        'end': dataset_test_process.time_series[ts_name].data}, features=TimeSeriesUtils.PLOT_FEATURES,
                    n_rows=2, n_cols=3, figsize=(15, 5), colors={'start': 'black', 'end': 'orange'}, save_plot=True,
                    subfolder=f"pca_{threshold_method}")

            test_errors = (np.square(
                dataset_test_process.time_series[ts_name].data - dataset_test.time_series[ts_name].data).mean(axis=1))

            test_anomalies_mask = None
            if threshold_method == 'quantile':
                test_anomalies_mask = ((test_errors > pc_upper_bound) | (
                        test_errors < pc_lower_bound)) == True
            elif threshold_method == 'svm':
                test_anomalies_mask = one_class_model.predict(
                    test_errors.reshape(-1, 1)) == -1
            elif threshold_method == 'local_outlier_factor':
                test_anomalies_mask = local_outlier_factor_model.predict(
                    test_errors.reshape(-1, 1)) == -1
            ...
            if test_anomalies_mask is not None:
                ...
                curr_traj_predicted_anomaly = np.count_nonzero(
                    test_anomalies_mask) > 0  # prediction
                ...
                anomalies = copy.deepcopy(
                    dataset_test.time_series[ts_name].data)
                anomalies[~test_anomalies_mask] = np.nan

                plot_ts(title=f"PCA Figure Test Anomalies {ts_name}", ts_name=f"test_anomalies_{ts_name}",
                        ts={'time_series': dataset_test.time_series[ts_name].data, 'anomalies': anomalies},
                        features=TimeSeriesUtils.PLOT_FEATURES,
                        n_rows=2, n_cols=3, figsize=(15, 5), colors={'time_series': 'black', 'anomalies': 'red'},
                        markers={'anomalies': 'o'}, save_plot=True, subfolder=f"pca_{threshold_method}")

                plot_scatter(timeseries_1=dataset_test.time_series[ts_name].data, timeseries_2=anomalies,
                             subfolder=f"pca_{threshold_method}", filename=ts_name, save_plot=True, show_plot=False,
                             verbose=True)

        save_stats_txt(
            tp, fn, fp, tn, subfolder=f"pca_{threshold_method}", filename=None, print_stats=True)


def main_nn_linear_regression(dataset_train, model, scaler_model, window_type, window_size, window_stride, device,
                              optimizer, loss_function, epoch, scheduler=None, dataset_eval=None, dataset_test=None):
    """
    ...
    if dataset_eval is not None:
        dataset_eval_process.add_window(
            type=window_type, window_size=window_size, stride=window_stride)
        dataset_eval_process.normalize(
            normalizer=scaler_model, mode='complete')
    ...
    loss_log = fit_nn_regression(model=model, x_training=x_training, y_training=y_training, x_eval=x_eval,
                                 y_eval=y_eval,
                                 optimizer=optimizer, scheduler=scheduler, loss_function=loss_function, epoch=epoch,
                                 print_epoch=True)
    ...
    if dataset_test is not None:
        ...
        for ts_name, t in dataset_test_process.time_series.items():
            ...
            for i, row_window in enumerate(t.data):
                ...
                anomalies_mask = ((error > pc_upper_bound) |
                                  (error < pc_lower_bound)) == True
                if anomalies_mask.any():
                    count_windows_anomaly += 1
                    errors_list.append(error)
                else:
                    anomalies[i * window_size:i *
                                              window_size + window_size] = np.nan
...
def main_svm(dataset_train, model, scaler_model, window_type, window_size, window_stride, with_pca, pca_components,
             show_plot_variance=True, dataset_eval=None, dataset_test=None):
    """
    ...
    if dataset_eval is not None:
        ...
        for ts_name, ts in dataset_eval_process.time_series.items():
            ...
            plot_ts(title=f"OneClass SVM {'PCA' if with_pca else 'NO PCA'} Figure Eval Anomalies {ts_name}",
                    ts_name=f"eval_anomalies_{ts_name}",
                    ts={'time_series': dataset_eval.time_series[ts_name].data, 'anomalies': anomalies},
                    features=TimeSeriesUtils.PLOT_FEATURES,
                    n_rows=2, n_cols=3, figsize=(15, 5), colors={'time_series': 'black', 'anomalies': 'red'},
                    markers={'anomalies': 'o'}, save_plot=True, subfolder=f"svm_{'pca' if with_pca else 'no_pca'}")
    ...
    if dataset_test is not None:
        ...
        for ts_name, ts in dataset_test_process.time_series.items():
            ...
            plot_ts(title=f"OneClass SVM {'PCA' if with_pca else 'NO PCA'} Figure Test Anomalies {ts_name}",
                    ts_name=f"test_anomalies_{ts_name}",
                    ts={'time_series': dataset_test.time_series[ts_name].data, 'anomalies': anomalies},
                    features=TimeSeriesUtils.PLOT_FEATURES,
                    n_rows=2, n_cols=3, figsize=(15, 5), colors={'time_series': 'black', 'anomalies': 'red'},
                    markers={'anomalies': 'o'}, save_plot=True, subfolder=f"svm_{'pca' if with_pca else 'no_pca'}")
            plot_scatter(timeseries_1=dataset_test.time_series[ts_name].data, timeseries_2=anomalies,
                         subfolder=f"svm_{'pca' if with_pca else 'no_pca'}", filename=ts_name, save_plot=True,
                         show_plot=False, verbose=True)

        save_stats_txt(
            tp, fn, fp, tn, subfolder=f"svm_{'pca' if with_pca else 'no_pca'}", filename=None, print_stats=True)


def main_local_outlier_factor(dataset_train, model, scaler_model, window_type, window_size, window_stride, with_pca,
                              pca_components, show_plot_variance=True, dataset_eval=None, dataset_test=None):
    """
    ...
    if dataset_eval is not None:
        ...
        for ts_name, ts in dataset_eval_process.time_series.items():
            ...
            plot_ts(title=f"Local Outlier Factor {'PCA' if with_pca else 'NO PCA'} Figure Eval Anomalies {ts_name}",
                    ts_name=f"eval_anomalies_{ts_name}",
                    ts={'time_series': dataset_eval.time_series[ts_name].data, 'anomalies': anomalies},
                    features=TimeSeriesUtils.PLOT_FEATURES,
                    n_rows=2, n_cols=3, figsize=(15, 5), colors={'time_series': 'black', 'anomalies': 'red'},
                    markers={'anomalies': 'o'}, save_plot=True,
                    subfolder=f"local_outlier_{'pca' if with_pca else 'no_pca'}")
    ...
    if dataset_test is not None:
        ...
        for ts_name, ts in dataset_test_process.time_series.items():
            ...
            plot_ts(title=f"Local Outlier Factor {'PCA' if with_pca else 'NO PCA'} Figure Test Anomalies {ts_name}",
                    ts_name=f"test_anomalies_{ts_name}",
                    ts={'time_series': dataset_test.time_series[ts_name].data, 'anomalies': anomalies},
                    features=TimeSeriesUtils.PLOT_FEATURES,
                    n_rows=2, n_cols=3, figsize=(15, 5), colors={'time_series': 'black', 'anomalies': 'red'},
                    ...
                    subfolder=f"local_outlier_{'pca' if with_pca else 'no_pca'}")
            plot_scatter(timeseries_1=dataset_test.time_series[ts_name].data, timeseries_2=anomalies,
                         subfolder=f"local_outlier_{'pca' if with_pca else 'no_pca'}", filename=ts_name, save_plot=True,
                         show_plot=False, verbose=True)

        save_stats_txt(
            tp, fn, fp, tn, subfolder=f"local_outlier_{'pca' if with_pca else 'no_pca'}", filename=None,
            print_stats=True)
