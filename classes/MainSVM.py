...
class MainSVM(Main):
    """
    Main class implementation for SVM
    """
    ...
    def __init__(self, params: Params = None) -> None:
        ...
        self.pca_model = None
        self.model = OneClassSVM(nu=0.01, kernel="rbf", gamma=0.0001)
        self.ad_params = {}
    ...
    def test(self, t_list: list = None, show_plot: bool = True,
        ...
        for ts_name, ts in self.dataset_test.time_series.items():
            ...
            plot_ts(
                title=f"One Class SVM {'PCA' if self.params.APPLY_PCA else 'NO PCA'} Figure Test Anomalies {ts_name}",
                ...
                subfolder=f"svm_{'pca' if self.params.APPLY_PCA else 'no_pca'}")
            try:
                plot_scatter(timeseries_1=ts.data, timeseries_2=anomalies,
                             subfolder=f"svm_{'pca' if self.params.APPLY_PCA else 'no_pca'}", filename=ts_name,
                             save_plot=True,
