...
def rosbag_to_csv(data_base_path=None):
    ...
    :param data_base_path: the path of the dataset folder
    :return: a list of the paths of the created csv files and a list of the paths of the created folders
    """
    ...
    for entry in os.scandir(data_base_path):
        if not entry.name.endswith(".bag"):
            continue
        ...
        if os.path.isfile(path):
            ...
            for topic in trajectory_bag.topic_table['Topics']:
                filepath = trajectory_bag.message_by_topic(
                    topic)  # save to csv by topic
...
def process_lidar_data(path_list):
    """
    For each file "robot-front_laser-scan.csv" containing the raw lidar data, create a file
    called "robot-processed_lidar.csv" with the following fields: timestamp, LS (lidar left), LC (lidar center), LD (lidar right)
    :param path_list: the list of folders in which there is the file robot-front_laser-scan.csv
    :return: none
    """
    for csv_filepath in path_list:
        ...
        for idx, row in read_csv.iterrows():
            ...
            LC = min(min(min(top_left_dist), 10), min(
                min(top_right_dist), 10))  # centro
        ...
        output_filepath = csv_filepath.replace(
            "front_laser-scan", "processed_lidar")
...
def create_ml_csv(dir_path_list):
    """
    For each folder create a file "ml_data_xxx.csv" containing the processed lidar data (LS, LC, LD) and the robot pose data (x, y, theta)
    this will be the file given to the machine learning model, hence the name "ml_data"
    :param path_list: the list of folders in which there is the file robot-front_laser-scan.csv
    :return: none
    """
    for curr_dir in dir_path_list:
        ...
        if len(combined_data["Time_lidar"]) > 0:
            ...
            new_x = [np.NaN] * len(combined_data["Time_lidar"])
            new_y = [np.NaN] * len(combined_data["Time_lidar"])
            new_theta = [np.NaN] * len(combined_data["Time_lidar"])
            new_time = [np.NaN] * len(combined_data["Time_lidar"])
            ...
            for i in range(len(combined_data["Time_pose"])):
                ...
                time_idx = np.argwhere(np.logical_and(
                    combined_data["Time_lidar"] >= bottom_limit, combined_data["Time_lidar"] <= up_limit))
...
def plot_data(path_list=None, show_plot=False, save_plot=False):
    """
    Plot the data contained in the csv files
    :param path_list: the list of csv files to plot
